...
public class ProductDAO {
    ...
    private static final String GET = "SELECT productID, name, price, quantity, picture, brand, status FROM tblProducts";
    ...
    private static final String SEARCH = "SELECT productID, name, price, quantity, picture, brand, status FROM tblProducts WHERE name LIKE ?";
    private static final String INSERT="INSERT INTO tblProducts (productID, name, price, quantity,picture,brand,status) VALUES(?,?,?,?,?,?,?) ";
    private static final String DELETE_PRODUCT = "DELETE FROM tblProducts WHERE productID =?";
    private static final String CHECK_STATUS = "SELECT status FROM tblProducts WHERE productID = ?";
        ...
        try {
            ...
            if (conn != null) {
                ...
                while(rs.next()) {
                    ...
                    list.add(new Product(productID, name, price, quantity, picture, brand,status ));
                }
            }
        } finally {
            if (rs != null) rs.close();
        ...
        try {
            ...
            rs= ptm.executeQuery();
                while (rs.next()) {
                    ...
                    listProduct.add(new Product(productID, name, price, quantity, picture, brand,status));
                }
            }
        }finally{
            if(rs!= null) rs.close();
...
public boolean checkInsert(Product product) throws ClassNotFoundException, SQLException, NamingException {
    boolean check = false;
    Connection conn = null;
    PreparedStatement ptm = null;
    ResultSet rs = null;
    ...
    try {
        ...
        if (conn != null) {
            ...
            ptm.setString(5, product.getPicture());
            ptm.setString(6, product.getBrand());
            ptm.setInt(7, 1);  // Inserting status as 1
